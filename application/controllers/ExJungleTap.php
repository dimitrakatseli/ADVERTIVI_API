...
class ExJungleTap extends CI_Controller {
	public function __construct()
		...
		public function index()
		{
			...
			$creativefilePath = './downloads/';
				...
				if($offer->Icon_url!=''){
					$iconPath = tempnam(sys_get_temp_dir(), 'php');
					file_put_contents($iconPath, file_get_contents($offer->Icon_url));
					$clogofile = new CURLFile($iconPath, filetype($iconPath), basename($iconPath));
					$data['logo']=$clogofile;
				}
				
				
				$jsonCreatives = file_get_contents("http://api.jungletap.com/affiliate/offer/getCreatives/".'?token='.$api_key.'&offer_id='.$offer->ID);
				$objCreatives=json_decode($jsonCreatives);
				
				if($objCreatives->success){
					if(isset($objCreatives->creatives)&&count($objCreatives->creatives)>0){
						foreach($objCreatives->creatives as $i=>$creative){
							$path_info = pathinfo($creative->url);
							$filenamefromserver=$creative->name;
							$filename=$creativefilePath.$advertiserId."_".$offer->ID.$filenamefromserver; // "bill"
							if($creativeimagecount!=5){							
								if($creative->type=='thumbfile'||$creative->type=='image'){
									if(file_exists($filename)){
										$creativeimagecount++;
										continue;
									}else{								
										file_put_contents($filename, file_get_contents($creative->url));

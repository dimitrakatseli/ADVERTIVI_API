...
class ApiTool extends CI_Controller {
	public function __construct()
        {		parent::__construct();
				$this->load->model('api_database');
			 
        }  
		public function index()
		{
			$demand_sources=$this->api_database->get_demand_sources(9);
			print_R($demand_sources);
			//$this->art_of_click();
		}
		
		public function art_of_click(){
			$_response='';
		...
		//CURL   
			...
			if($_response!=''&&$_response->count>0){
				...
				if(count($offerTolocalAdd)>0){	
					$addedTolocal=$this->api_database->addTolocalDb($offerTolocalAdd);
				}
				if(count($offerTolocalForEdit)>0){
					$updatedTolocal=$this->api_database->updateTolocalDb($offerTolocalForEdit);
					 
				}
				if(count($deletedOffersFromAffise)>0){
					$updatedTolocalAffiseDelete=$this->api_database->updateTolocalDbAffiseDelete($deletedOffersFromAffise);
					
				}
				if(count($allOffersFromAdvertiser)>0){
					$deletedFromLocal=$this->api_database->deleteFromLocal($allOffersFromAdvertiser);
					
				}
				//api_database->updateDemandSource($allOffersFromAdvertiser);		
			}
		}
		...
		public function checkExistingOffer($advertOfferId){
			$oldRecord=$this->api_database->checkExistingOffer_db($advertOfferId);
			return $oldRecord;
		}
}

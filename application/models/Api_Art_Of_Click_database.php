...
Class Api_Art_Of_Click_Database extends CI_Model {
	...
	public function getExistingOffer_db($advertiserId) {
		...
		$oldRecord=array();
		$condition = "advertiser='".$advertiserId."' and deletedFromAffise!=1";
		$this->db->select('id,affise_offer_id,advertiser_offer_id,deletedFromAffise,status,title,daily_cap,description,payments');
		$this->db->from('tbl_offer');
		$this->db->where($condition);
		$query = $this->db->get();
	...
	public function addTolocalDb($sourceId,$offerTolocalAdd,$advertiserId) {
		...
		foreach($offerTolocalAdd as $offerTolocal){
			if($Addvals!=''){
				$Addvals=$Addvals.',('.$offerTolocal->affiseofferid.','.$offerTolocal->id.','.$sourceId.',"'.urlencode($offerTolocal->name).'","'.$offerTolocal->advertiser.'","'.urlencode($offerTolocal->trackingUrl).'","'.urlencode($offerTolocal->previewUrl).'","","","'.urlencode($offerTolocal->description).'","","'.implode("|",$offerTolocal->countries).'","","Art Of Click","","'.$offerTolocal->status.'","","","","'.$offerTolocal->payout.'","","","","'.($offerTolocal->dailyCap).'","","","","","'.implode("|",$offerTolocal->os).'","","","","","","","","","","","","","","","")';
			}else{
				$Addvals=$Addvals.'('.$offerTolocal->affiseofferid.','.$offerTolocal->id.','.$sourceId.',"'.urlencode($offerTolocal->name).'","'.$offerTolocal->advertiser.'","'.urlencode($offerTolocal->trackingUrl).'","'.urlencode($offerTolocal->previewUrl).'","","","'.urlencode($offerTolocal->description).'","","'.implode("|",$offerTolocal->countries).'","","Art Of Click","","'.$offerTolocal->status.'","","","","'.$offerTolocal->payout.'","","","","'.($offerTolocal->dailyCap).'","","","","","'.implode("|",$offerTolocal->os).'","","","","","","","","","","","","","","","")';
			}
		}

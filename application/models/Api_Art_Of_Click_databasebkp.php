<?php
Class Api_Art_Of_Click_Database extends CI_Model {
	...
	public function checkExistingOffer_db($advertOfferId) {
		$oldRecord=array();
		$condition = "advertiser_offer_id =".$advertOfferId." and deletedFromAffise!=1";
		$this->db->select('*');
	...
	public function addTolocalDb($sourceId,$offerTolocalAdd) {
		...
		foreach($offerTolocalAdd as $offerTolocal){
			if($Addvals!=''){
				$Addvals=$Addvals.',('.$offerTolocal->affiseofferid.','.$offerTolocal->id.','.$sourceId.',"'.urlencode($offerTolocal->name).'","'.$offerTolocal->advertiser.'","'.urlencode($offerTolocal->trackingUrl).'","'.urlencode($offerTolocal->previewUrl).'","","","'.urlencode($offerTolocal->description).'","","'.implode("|",$offerTolocal->countries).'","","Art Of Click","","'.$offerTolocal->status.'","","","","'.$offerTolocal->payout.'","","","","'.urlencode($offerTolocal->dailyCap).'","","","","","'.implode("|",$offerTolocal->os).'","","","","","","","","","","","","","","","")';
			}else{
				$Addvals=$Addvals.'('.$offerTolocal->affiseofferid.','.$offerTolocal->id.','.$sourceId.',"'.urlencode($offerTolocal->name).'","'.$offerTolocal->advertiser.'","'.urlencode($offerTolocal->trackingUrl).'","'.urlencode($offerTolocal->previewUrl).'","","","'.urlencode($offerTolocal->description).'","","'.implode("|",$offerTolocal->countries).'","","Art Of Click","","'.$offerTolocal->status.'","","","","'.$offerTolocal->payout.'","","","","'.urlencode($offerTolocal->dailyCap).'","","","","","'.implode("|",$offerTolocal->os).'","","","","","","","","","","","","","","","")';
			}
		}
	...
	public function updateTolocalDb($offerTolocalForEdit) {
		...
		public function updateTolocalDbEditStatus($offerTolocalForEditStatus) {
		
		$queryToUpdateStatusOnLocal = "update tbl_offer set status='stopped' where id in(".implode(",",$offerTolocalForEditStatus).")";
	...
	public function getExtraFromLocal($allOffersFromAdvertiser) {
		$condition="advertiser_offer_id NOT IN(".implode(",",$allOffersFromAdvertiser).") and status='active'";
		$this->db->select('*');
		$this->db->from('tbl_offer');
		$this->db->where($condition);
		$query = $this->db->get();
	...
	public function updateDemandSource_lastUpdate(){
		$now = date('Y-m-d H:i:s');

		$queryTolastupdate = "update tbl_demand_sources set last_update='".$now."'";
